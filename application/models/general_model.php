...
class General_model extends CI_Model{
  ...
  public function regis($nama,$username,$email,$password,$gender,$tgl_lahir){
    ...
    $checking = $this->db->insert('user',['reviewerID'=>$hasil,'Nama_user'=>$nama,'Username'=>$username,'Email'=>$email,'Password'=>$password,'Gender'=>$gender,'Tanggal_lahir'=>$tgl_lahir,'Status'=>'member']);
    if($checking){
      return TRUE;
    }else{
      return FALSE;
  ...
  public function preference(){
    $query = "select * from metadata where rate='0' order by rand()";
    return $this->db->query($query);
  }
